...
from prompt_service import get_patient_prompt, format_chat_history, CONDENSE_QUESTION_PROMPT, \
    update_patient_info
...
def main():
    ...
    # with st.sidebar.expander("Сицальный анамнез "):
    #     st.text_area("",
    ...
    with st.sidebar.expander("Пациент "):
        system_prompt = st.text_area("Профиль пациента :",
                                     ...
                                     "Ты работаешь кочегаром в котельной городской больницы, "
                                     "работа очень тяжелая и в последние несколько месяцев "
                                     "тебе очень тяжело физически трудиться. "
                                     "У тебя избыточный вес, ты пробовал разные диеты, "
                                     "но они не помогают его сбросить. "
                                     "Ты любишь проводить время с семьей на природе и пикники. "
